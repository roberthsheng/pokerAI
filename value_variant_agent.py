...
class ValueVariantAgent:
    ...
    def step(self, observation):
        ...
        if mask[2] == 1:
            ...
            select = np.random.choice([0, 1], p=[0.2, 0.8])
            if hand_equity >= pot_odds and select == 1:
                options.append(2)
        ...
        if mask[3] == 1:
            ...
            select = np.random.choice([0, 1], p=[0.3, 0.7])
            if hand_equity >= pot_odds and select == 1:
                options.append(3)
        ...
        if mask[4] == 1 and stack > state['amount_to_play'] + state['pot']:
            ...
            select = np.random.choice([0, 1], p=[0.4, 0.6])
            if hand_equity < pot_odds and select == 1:
                options.append(4)
